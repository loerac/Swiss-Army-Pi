...
static bool validWireless(const char *const ifname) {
   ...
   struct iwreq pwrq;
   bool ok = false;
   imemset(&pwrq, 0, sizeof(pwrq));
   istrncpy(pwrq.ifr_name, ifname, sizeof(pwrq.ifr_name));
   ...
   if (getifaddrs(&ifaddr) == -1) {
      ...
      for (ifa = ifaddr; ifa != NULL; ifa = ifa->ifa_next) {
         ...
         if (validWireless(ifa->ifa_name)) {
            istrncpy(iwname,ifa->ifa_name, strlen(ifa->ifa_name));
            ok = true;
         }
      }
      freeifaddrs(ifaddr);
...
static bool getSignalInfo(signal_info_s *const sig_info, const char *const iwname) {
   ...
   istrncpy(req.ifr_name, iwname, sizeof(req.ifr_name));
   ...
   if (ok) {
      ...
      if (ioctl(sockfd, SIOCGIWESSID, &req) == -1) {
         ...
         imemcpy(&sig_info->ssid, req.u.essid.pointer, req.u.essid.length);
         imemset(&sig_info->ssid[req.u.essid.length], 0, 1);
      }
   }
   (void)close(sockfd);
